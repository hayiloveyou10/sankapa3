...
import { Home, Users, Gift, User, Shield, TreePine, Coins } from 'lucide-react';
...
const BottomNav: React.FC = () => {
  ...
  // Additional nav items for new features
  const additionalItems = [
    {
      icon: TreePine,
      label: 'Forest',
      path: '/community-forest',
      isActive: location.pathname === '/community-forest'
    },
    {
      icon: Coins,
      label: 'Store',
      path: '/coin-store',
      isActive: location.pathname === '/coin-store'
    }
  ];
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-2 z-40">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Additional navigation row for new features */}
        <div className="flex justify-center items-center space-x-8 mt-2 pt-2 border-t border-gray-100">
          {additionalItems.map((item) => (
            ...
            >
              <item.icon className={`w-5 h-5 mb-1 ${item.isActive ? 'text-green-600' : 'text-gray-400'}`} />
              <span className="text-xs font-medium">{item.label}</span>
            </motion.button>
          ))}
        </div>
