...
import { collection, addDoc, runTransaction, doc, updateDoc, serverTimestamp } from 'firebase/firestore';
...
interface Reward {
  ...
  type: 'theme' | 'badge' | 'tree-design' | 'recharge' | 'donation' | 'music' | 'feature' | 'certificate' | 'hero-status';
    ...
    >
      ...
      <div className="relative mb-4">
        <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-3 ${
          ...
          reward.type === 'certificate' ? 'bg-gradient-to-br from-indigo-400 to-purple-500' :
          reward.type === 'hero-status' ? 'bg-gradient-to-br from-yellow-400 to-orange-500' :
      ...
      {isUnlocked ? (
        <div className="space-y-2">
          {reward.category === 'in-app' && onActivate && reward.type !== 'certificate' && (
            <motion.button
          ...
          {reward.type === 'certificate' && onUse && (
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={() => onUse(reward)}
              className="w-full py-3 px-4 rounded-xl font-medium bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:from-indigo-700 hover:to-purple-700 shadow-lg hover:shadow-xl transition-all duration-200"
            >
              <div className="flex items-center justify-center space-x-2">
...
const Rewards: React.FC = () => {
  ...
  const inAppRewards: Reward[] = [
    ...
    {
      id: 'resolution-certificate',
      name: 'সঙ্কল্পপত্র',
      description: 'আপনার অর্জনের জন্য একটি সুন্দর সার্টিফিকেট ডাউনলোড করুন।',
      cost: 2000,
      icon: Certificate,
      category: 'in-app',
      type: 'certificate',
    ...
    {
      id: 'community-hero',
      name: 'কমিউনিটি হিরো (২৪ ঘন্টা)',
      description: 'বিশেষ ক্ষমতা পান: পোস্ট পিন করুন, হিরো পুরস্কার দিন এবং বিশেষ ব্যাজ পান।',
      cost: 10000,
      icon: Crown,
      category: 'in-app',
      type: 'hero-status',
  ...
  const realWorldRewards: Reward[] = [
    ...
    {
      id: 'plant-tree',
      name: 'কমিউনিটি ফরেস্টে গাছ লাগান',
      description: 'ভার্চুয়াল কমিউনিটি ফরেস্টে আপনার নামে একটি গাছ লাগান।',
      cost: 1000,
      icon: TreePine,
      category: 'real-world',
      type: 'donation'
  ...
  const handleRedeemReward = async (reward: Reward) => {
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        if (reward.type === 'hero-status') {
          // Community Hero status
          const expiresAt = new Date();
          expiresAt.setHours(expiresAt.getHours() + 24);
          
          updateData.isCommunityHero = true;
          updateData.heroStatusExpiresAt = expiresAt;
        } else if (reward.category === 'in-app' && reward.unlockCategory) {
          // Regular in-app rewards
        ...
        if (reward.category === 'real-world') {
          if (reward.id === 'plant-tree') {
            // Add tree to community forest
            const forestRef = doc(collection(db, 'communityForest'));
            transaction.set(forestRef, {
              planterUsername: userData.displayName,
              plantedAt: new Date(),
              treeType: 'oak'
            });
          } else {
            // Other real-world rewards
      ...
      if (reward.category === 'real-world') {
        if (reward.id === 'plant-tree') {
          toast.success('গাছ লাগানো হয়েছে! কমিউনিটি ফরেস্ট দেখুন।');
        } else {
  ...
  const handleUseCertificate = (reward: Reward) => {
            ...
            {inAppRewards.map((reward, index) => (
              ...
              >
                <RewardCard
                  ...
                  onUse={reward.type === 'certificate' ? handleUseCertificate : undefined}
                />
              </motion.div>
            ))}
          </div>
